...
class UserController {
  static async register(req, res) {
    ...
    try {
      ...
      res.status(201).render('login',{error: 'User registered successfully' });
    } catch (error) {
      console.error(error);
  ...
  static async login(req, res) {
    ...
    try {
      ...
      const token = jwt.sign({ userId: user.id }, process.env.JWT_SECRET, { expiresIn: '1h' }); //cria o token caso tudo esteja certo, user e senha

      res.cookie('token', token, { httpOnly: true }); //nome da chave, o token feito, a sessão ativa
      res.status(200).render('secure',{ username: user.username });
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: 'Internal Server Error' });
    }
  ...
  static async logout(req, res) {
    ...
    res.status(200).render('login',{error: 'Logout successful'});
  }
}
